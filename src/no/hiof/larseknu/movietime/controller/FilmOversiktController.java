...
public class FilmOversiktController {
    ...
    @FXML
    private Label tittelText;
    @FXML
    private TextField utgivelsesdatoTextField, spilletidTextField;
    @FXML
    private TextArea beskrivelseTextArea;
    @FXML
    private Button redigerButton, tittelStigende, tittelSynkende, aarSynkende, aarStigende;
    ...
    private void initialize() {
        ...
        tittelStigende.setOnAction(new EventHandler<ActionEvent>() {
            @Override
            public void handle(ActionEvent event) {
                Collections.sort(MainJavaFX.mainJavaFX.getFilmData(), new Comparator<Film>() {
                    @Override
        ...
        tittelSynkende.setOnAction(new EventHandler<ActionEvent>() {
            @Override
            public void handle(ActionEvent event) {
                Collections.sort(MainJavaFX.mainJavaFX.getFilmData(), new Comparator<Film>() {
                    @Override
                    public int compare(Film o1, Film o2) {
                        return o2.getTittel().compareTo(o1.getTittel());
                    }
                });
            }
        });

        aarStigende.setOnAction((new EventHandler<ActionEvent>() {
            @Override
            public void handle(ActionEvent event) {
                Collections.sort(MainJavaFX.mainJavaFX.getFilmData(), new Comparator<Film>() {
                    @Override
        ...
        aarSynkende.setOnAction(new EventHandler<ActionEvent>() {
            @Override
            public void handle(ActionEvent event) {
                Collections.sort(MainJavaFX.mainJavaFX.getFilmData(), new Comparator<Film>() {
                    @Override
        ...
        filmListView.getSelectionModel().selectedItemProperty().addListener(new ChangeListener<Film>() {
            ...
            public void changed(ObservableValue<? extends Film> observable, Film gammelFilm, Film nyFilm) {
                ...
                if (nyFilm != null) {
                    Image image = new Image("https://image.tmdb.org/t/p/w500" + nyFilm.getPosterURL());
                    bilde.setImage(image);
                }
            }
        });
    ...
    public void nyButtonClicked(ActionEvent actionEvent) {
        ...
        boolean nyFilmLagetVellykket = mainApp.visRedigerFilmDialog(nyFilm);

        // Sjekker om den nye filmen ble laget
        if (nyFilmLagetVellykket) {
    ...
    public class FilmCelle extends ListCell<Film> {
        @Override
